...
{
    public partial class UC_Master2 : UserControl
    {
        ...
        {
            InitializeComponent();
            //btnClose.ImageForBtn(Resources.clearheaderandfooter);
            //btnSave.ImageForBtn(Resources.save);
            //btnReset.ImageForBtn(Resources.reset);
            //bindingNavigator.Items.OfType<ToolStripButton>().ForEach(x =>
            //{
            //    x.Margin = new Padding(3, 6, 3, 6);
        ...
        //private void DataLayoutControl1_HideCustomization(object sender, EventArgs e) => ClsPath.SaveCustomContol(this.dataLayout, CurrType?.Name);
        ...
        {
            // LinqModel.DataClasses1DataContext.ClearTrackedItems();
        }
        ...
        {
            if (IsNew)
                dxValidationProvider.Validate(nameEdit);
        }
        bool ValidateData()
        {
            if (nameEdit?.EditValue != null && nameEdit?.Text.Trim() == String.Empty)
                nameEdit.EditValue = null;
            return dxValidationProvider.Validate();
        }
      LinqModel.DataClasses1DataContext dbLocal = new  LinqModel.DataClasses1DataContext(Program.ConnectionString);
        public virtual void Save()
        {
            try
            {
                ...
                EnableValidation();
                if (this.ValidateChildren() == false) return;
                DisableValidation();
                BeforeSave();
                //using (var dbLocal = new  LinqModel.DataClasses1DataContext(Program.ConnectionString))
                //{
                //if (IsNew)
                //    dbLocal.GetTable(CurrType).InsertOnSubmit(bindingSource.Current);
                //else
                //{
                //    dbLocal.GetTable(CurrType).DeleteOnSubmit(bindingSource.Current);
                //    dbLocal.GetTable(CurrType).InsertOnSubmit(bindingSource.Current);
                //}
                dbLocal.SubmitChanges();
                SetBalance();
                flyDialog.ShowDialogUC(this, MessgTextSave, this.IsNew);
                //}
                RefreshData();
            }
            catch (Exception ex)
        ...
        {
            dbLocal.GetTable(bindingSource.Current.GetType()).InsertOnSubmit(bindingSource.Current);
        ...
        void TextReadOnly(bool state)
        {
            if (dataLayout != null)
                dataLayout.OptionsView.IsReadOnly = state ? DefaultBoolean.True : DefaultBoolean.False;
        }
        void InitBraAndUserForNew()
        {

            try
            {
                dataLayout?.Items?.OfType<LayoutControlItem>()?.Where(x => x.Control is BaseEdit).ForEach(x =>
                {
                    if (x.Control is BaseEdit edit)
                        switch (edit?.DataBindings[0]?.BindingMemberInfo.BindingMember)
                        {
                            case "BranchID":
                            case "BrnId":
                                edit.EditValue = Program.Branch.ID;
                                break;
                            case "Name":
                                edit.Focus();
                                break;
                            case "UserID":
                                edit.EditValue = Program.User.ID;
                                break;
                            case "EnterTime":
        ...
        {
            if (bindingSource.Current == null) return;
            if (ClsXtraMssgBox.ShowWarningYesNo(MessgTextDelete) != DialogResult.Yes) return;
            //using ( LinqModel.DataClasses1DataContext db = new  LinqModel.DataClasses1DataContext(Program.ConnectionString))
            //{
                dbLocal.GetTable(bindingSource.Current.GetType()).DeleteOnSubmit(bindingSource.Current);
                dbLocal.SubmitChanges();
            //    db.GetTable(bindingSource.Current.GetType()).DeleteOnSubmit(bindingSource.Current);
            //    db.SubmitChanges();
            //}
            flyDialog.ShowDialogUCUdeletedMsg(this, MessgTextSave);
            RefreshData();
        }
       
        //internal virtual bool CheckIfObjectCanBeDeleted(PaymentMethods paymentMethods)
        //{
        //    using ( LinqModel.DataClasses1DataContext db = new  LinqModel.DataClasses1DataContext(Program.ConnectionString))
        //    {
        //        var entrys = db.EntryDetails.Where(x => x.AccountID == paymentMethods.AccountID
        //        && x.Entry.ProcessType != SystemProcess.AccountOpenBalance).Count();
        //        return entrys == 0;
        //    }
        //}
        ...
        public virtual void RefreshData()
        {
            try
            {
                ...
                TextReadOnly(true);
                dbLocal = new  LinqModel.DataClasses1DataContext(Program.ConnectionString);
                if (bindingSource.Current != null)
                {
                    object v = bindingSource.Current;
                    bindingSource.Clear();
                    bindingSource.DataSource = dbLocal.GetTable(v.GetType());
                }
        ...
        private void UC_MasterAll_Load(object sender, EventArgs e)
        {
            ...
            if (nameEdit != null)
            {
                ValidationProvider(nameEdit);
                nameEdit.EditValueChanged += NameTextEdit_EditValueChanged;
            }
        }
        ...
        private void btnClose_Click(object sender, EventArgs e) => this.Parent.Dispose();
    }
