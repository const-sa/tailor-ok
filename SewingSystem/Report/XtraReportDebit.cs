...
{
    public partial class XtraReportDebit : DevExpress.XtraReports.UI.XtraReport
    {
        string Tax_Number = "";
        public XtraReportDebit()
        {
            InitializeComponent();
        ...
        public static void Print(object ds,long id,string ThePay,string sellDate)
        {
            ...
            if (id > 0)
            {
                ...
                {
                    short pri = ((db.tblNotes.SingleOrDefault(p => p.ID == id).HowPrint as short?) ?? 0);
                    db.tblNotes.SingleOrDefault(p => p.ID == id).HowPrint = ++pri;
                    db.SubmitChanges();
                }
            }
