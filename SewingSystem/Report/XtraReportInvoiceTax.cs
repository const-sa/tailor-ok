...
{
    public partial class XtraReportInvoiceTax : DevExpress.XtraReports.UI.XtraReport
    {
        string InvType;
        public XtraReportInvoiceTax(string type)
        {
            InitializeComponent();
            InvType = type;
        }
        ...
        private void XtraReportInvoiceAllColumns_BeforePrint(object sender, CancelEventArgs e)
        {
            ...
            if (InvType == Program.Buy | InvType == Program.ReturnBuy)
            {
            //xrCusOrSup.Text= "اسم المورد";
            //    if (InvType == Program.Buy)
            //        xrTaitelInvo.Text = "تقارير المشتريات للفترة من ";
            //    else if (InvType == Program.ReturnBuy)
            //        xrTaitelInvo.Text = "تقارير مردودات المشتريات للفترة من";
            }
            else if (InvType == Program.Sale | InvType == Program.ReturnSale)
            {
                ////xrCusOrSup.Text = "اسم العميل";
                ////if (InvType == Program.Sale)
                ////    xrTaitelInvo.Text = "تقارير المبيعات للفترة من";
                ////else if (InvType == Program.ReturnSale)
                ////    xrTaitelInvo.Text = "تقارير مردودات المبيعات للفترة من";
            }
        }
        ...
        private void bindingSource1_CurrentChanged(object sender, EventArgs e)
        {
            ...
            {
                foreach (tblSaleInvoice item in bindingSource1)
                {
                    item.TheTax = 0;
                }
                //if (bindingSource1.Current!=null)
                //{
                //  tblSaleInvoice inv = bindingSource1.Current as tblSaleInvoice;
                //inv.TheTax = 0;
                //}
            }
        }
